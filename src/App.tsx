...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-3 md:px-6 md:py-4 flex flex-row justify-between items-center gap-3 sticky top-0 z-40 shadow-sm" id="main-header">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-lg md:text-2xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2">
              NutriLens <span className="hidden sm:inline font-normal text-slate-400">| Smart Grocery Reader</span>
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-[10px] md:text-xs font-bold border border-emerald-200 whitespace-nowrap">
            Pendamping Lansia
          </div>
        </div>
      </header>
          ...
          <AnimatePresence mode="wait">
            {analysisResult && !isAnalyzing ? (
              ...
              >
                ...
                <div
                  className={`bg-white rounded-3xl p-5 sm:p-8 border-4 shadow-sm relative overflow-hidden flex flex-col items-center text-center justify-center ${
                ...
                >
                  {/* Verdict Badge Icon */}
                  <div
                    className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mb-4 border-4 shadow-inner ${
                      analysisResult.color === "merah"
                  ...
                  <h2
                    className={`text-2xl sm:text-4xl font-black uppercase tracking-tight mb-1 ${
                      analysisResult.color === "merah"
                        ? "text-red-600"
                        : analysisResult.color === "kuning"
                        ? "text-amber-500"
                  ...
                  <p className="text-lg sm:text-xl font-bold text-slate-700 mb-4" id="verdict-product-name">
                    {analysisResult.productName || "Nama Produk Makanan"}
                  </p>

                  <div className="w-full h-px bg-slate-100 mb-4"></div>

                  <p className="text-sm sm:text-base text-slate-700 leading-relaxed max-w-lg mb-6" id="verdict-reason">
                  ...
                  <div className="space-y-3" id="nutrients-grid-list">
                    {analysisResult.nutrients && analysisResult.nutrients.length > 0 ? (
                      analysisResult.nutrients.map((nut, index) => (
                        <div
                          key={index}
                          className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-4 bg-slate-50 rounded-2xl border border-slate-150 gap-3"
                        >
                          <div>
      ...
      <footer className="mt-8 text-center text-xs text-slate-400 max-w-2xl mx-auto px-4 pb-20 md:pb-0" id="app-footer">
        <div className="w-16 h-px bg-slate-200 mx-auto mb-4" />
        <p className="font-medium text-slate-500">© 2026 NutriLens Smart Grocery Reader - Oleh Hardianto.dev</p>
        <p className="mt-1">Dibuat khusus untuk mendukung kemandirian hidup sehat kakek, nenek, dan lansia Indonesia.</p>
      </footer>

      {/* Fixed bottom navigation menu for mobile devices */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 py-3 px-6 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] flex justify-around items-center z-50 md:hidden" id="mobile-nav-bar">
        <button
          onClick={() => {
            const el = document.getElementById("section-restrictions");
            el?.scrollIntoView({ behavior: "smooth", block: "center" });
          }}
          className="flex flex-col items-center gap-1 text-slate-500 active:text-emerald-600 transition-colors cursor-pointer"
        >
          <span className="text-lg">🩺</span>
          <span className="text-[10px] font-bold text-slate-600">Kondisi</span>
        </button>
        <button
          onClick={() => {
            const el = document.getElementById("section-camera");
            el?.scrollIntoView({ behavior: "smooth", block: "center" });
          }}
          className="flex flex-col items-center gap-1 text-slate-500 active:text-emerald-600 transition-colors cursor-pointer"
        >
          <span className="text-lg">📷</span>
          <span className="text-[10px] font-bold text-slate-600">Kamera</span>
        </button>
        <button
          onClick={() => {
            const el = document.getElementById("active-result-card") || document.getElementById("welcome-instruction");
            el?.scrollIntoView({ behavior: "smooth", block: "center" });
          }}
          className="flex flex-col items-center gap-1 text-slate-500 active:text-emerald-600 transition-colors cursor-pointer"
        >
          <span className="text-lg">📊</span>
          <span className="text-[10px] font-bold text-slate-600">Hasil</span>
        </button>
        <button
          onClick={() => {
        ...
        >
          <span className="text-lg">⏳</span>
          <span className="text-[10px] font-bold text-slate-600">Riwayat</span>
        </button>
      </div>
    </div>
  );
